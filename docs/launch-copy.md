# Launch Copy

This file contains ready-to-edit launch copy for:

- GitHub release notes
- LinkedIn launch post

## GitHub Release Copy

### Title

`ai-os-framework v0.1.0: control plane, contracts, and a War Room starter`

### Body

Today I'm open-sourcing `ai-os-framework`.

This repository is a starter for building AI systems that need more than prompt workflows and isolated agent sessions.

The core idea is simple:

- one control plane
- multiple domain subsystems
- explicit subsystem contracts
- a unified War Room / operator surface
- health, observability, and recovery built in

Why this matters:

Most AI projects can show isolated intelligence.
Much fewer can show how an operator or executive should actually run the system once multiple agents, domains, and risks start interacting.

`ai-os-framework` is designed for that operating layer.

Included in this first release:

- control-plane repo structure
- contract schemas
- subsystem templates
- example subsystem outputs
- bootstrap script
- demo landing page
- minimal War Room demo
- executive-facing visual assets

What this repo is not:

- not a private runtime dump
- not a personal memory vault
- not a monolithic agent toy

It is a public-friendly framework layer you can use to build your own AI operating system.

Start here:

- README
- `demo/index.html`
- `demo/war_room/index.html`

## LinkedIn Launch Copy

Most AI projects don't fail because the models are weak.

They fail because the system becomes hard to operate once you have:

- multiple agents
- multiple domains
- hidden state
- unclear priorities
- no shared operator surface

That's the problem I wanted to address with `ai-os-framework`, which I open-sourced today.

It is a starter for building AI operating systems with:

- one control plane
- multiple domain subsystems
- contract-first integration
- a War Room for operators
- health, observability, and recovery patterns

The goal is not to make AI look impressive in isolated sessions.
The goal is to make it legible under pressure.

That means an operator should be able to answer:

- what matters now
- what is blocked
- which subsystem is drifting
- what should happen next

This first public version includes:

- framework structure
- contract schemas
- templates
- example subsystem outputs
- demo landing page
- minimal War Room demo

If you're working on multi-agent systems, operator tooling, or AI infrastructure that needs real governance, this may be useful.

Repo:

`git@github.com:Kennysu0425/ai-os-framework.git`

Suggested closing line:

Build a control plane, not just another pile of agent scripts.
